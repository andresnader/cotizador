...
function loadExternalScriptOnce(src, readinessCheck) {
    ...
    const promise = new Promise((resolve, reject) => {
        ...
        const scheduleTimeoutGuard = (scriptEl) => {
            ...
            const poll = () => {
                if (finished) return;
                if (isReady()) {
                    markAsLoaded(scriptEl);
                    return;
                }
                elapsed += pollInterval;
                if (elapsed >= timeoutMs) {
                    finished = true;
                    reject(new Error(`Tiempo de espera agotado al cargar el script externo: ${src}`));
                    return;
                }
        ...
        if (existingScript) {
            ...
            scheduleTimeoutGuard(existingScript);
            return;
        }
...
function getPdfLibrariesIfReady() {
...
function waitForCondition(predicate, { timeoutMs = 8000, intervalMs = 100 } = {}) {
    return new Promise((resolve, reject) => {
        ...
        const check = () => {
            ...
            if (Date.now() - start >= timeoutMs) {
                reject(new Error('Tiempo de espera agotado al preparar dependencias externas.'));
                return;
            }
...
async function ensurePdfLibrariesAvailable() {
    const readyLibraries = getPdfLibrariesIfReady();
    if (readyLibraries) {
        return readyLibraries;
    }

    if (!pdfLibrariesPromise) {
        pdfLibrariesPromise = (async () => {
            await loadExternalScriptOnce(HTML2CANVAS_CDN_URL, resolveHtml2canvasFunction);
            await waitForCondition(() => typeof resolveHtml2canvasFunction() === 'function');

            await loadExternalScriptOnce(JSPDF_CDN_URL, resolveJsPdfConstructor);
            await waitForCondition(() => typeof resolveJsPdfConstructor() === 'function');

            const libraries = getPdfLibrariesIfReady();
            if (!libraries) {
                throw new Error('Las librerías de generación de PDF no están disponibles.');
            }
            return libraries;
        })();
    }
    ...
    try {
        return await pdfLibrariesPromise;
    } catch (error) {
...
async function generatePrintableQuote() {
    ...
    try {
        ...
        const rawMessage = err?.result?.error?.message || err?.message || 'Error desconocido.';
        let friendlyMessage = rawMessage;
        if (/html2canvas|jspdf|Tiempo de espera/i.test(rawMessage)) {
            friendlyMessage = 'No se pudieron preparar las librerías para crear el PDF. Verifica la conexión a internet o que los scripts de html2canvas y jsPDF estén disponibles.';
        }
        showAlert(`Error al generar/guardar la cotización: ${friendlyMessage}`, 'Error al Guardar');
