...
function loadExternalScriptOnce(src, readinessCheck) {
    ...
    const promise = new Promise((resolve, reject) => {
        let scriptEl = Array.from(document.getElementsByTagName('script')).find(script => script.src === src);
        ...
        const checkReadiness = () => {
            if (typeof readinessCheck !== 'function') return false;
            try {
        ...
        const cleanup = () => {
            if (scriptEl) {
                if (onLoadListener) {
                    scriptEl.removeEventListener('load', onLoadListener);
                }
                if (onErrorListener) {
                    scriptEl.removeEventListener('error', onErrorListener);
                }
            }
            if (pollIntervalId) {
        ...
        const attemptImmediateResolve = () => {
            if (scriptEl) {
                const readyState = scriptEl.readyState;
                if (scriptEl.dataset.loaded === 'true' || readyState === 'complete' || readyState === 'loaded') {
                    resolvePromise();
                    return true;
                }
            }
        ...
        onErrorListener = () => {
            rejectPromise(new Error(`No se pudo cargar el script externo: ${src}`));
        };

        const startWatchers = () => {
            if (!scriptEl) {
                rejectPromise(new Error(`No se pudo crear la etiqueta de script para ${src}`));
                return;
            }

            scriptEl.addEventListener('load', onLoadListener, { once: true });
            scriptEl.addEventListener('error', onErrorListener, { once: true });

            if (!scriptEl.isConnected) {
                document.head.appendChild(scriptEl);
            }

            if (attemptImmediateResolve()) {
                return;
            }
            ...
            timeoutId = setTimeout(() => {
                ...
                rejectPromise(new Error(`Tiempo de espera agotado al cargar el script externo: ${src}`));
            }, 15000);
        };
